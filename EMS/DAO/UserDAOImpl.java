...
public class UserDAOImpl implements UserDAO {
		...
		public Login validateUser(Login login) {
			String sql = "SELECT count(LOGIN_ID) FROM EIS_MST_LOGIN_ACCESS WHERE LOGIN_NAME = ? and PASSWORD= ?";
			int count = jdbcTemplate.queryForObject(sql, Integer.class, login.getLOGIN_NAME(), login.getPASSWORD());
			if(count == 0) {
	    		        return null;
			} else {
				return login;
